...
#else
// We have to hook some native APIs, as third-party network libraries may block
// the coroutine schedulers.
...
typedef int(WINAPI *sendto_fp_t)(SOCKET, CONST char *, int, int,
                                 CONST sockaddr *, int);
typedef int(WINAPI *select_fp_t)(int, fd_set *, fd_set *, fd_set *,
                                 const timeval *);

typedef void(WINAPI *Sleep_fp_t)(DWORD);
typedef SOCKET(WINAPI *WSASocketA_fp_t)(int, int, int, LPWSAPROTOCOL_INFOA,
                                        GROUP, DWORD);
typedef SOCKET(WINAPI *WSASocketW_fp_t)(int, int, int, LPWSAPROTOCOL_INFOW,
                                        GROUP, DWORD);
typedef int(WINAPI *WSAIoctl_fp_t)(SOCKET, DWORD, LPVOID, DWORD, LPVOID, DWORD,
...
typedef SOCKET(WINAPI *WSAAccept_fp_t)(SOCKET, sockaddr *, LPINT,
                                       LPCONDITIONPROC, DWORD_PTR);
typedef int(WINAPI *WSAConnect_fp_t)(SOCKET, CONST sockaddr *, int, LPWSABUF,
                                     LPWSABUF, LPQOS, LPQOS);
typedef int(WINAPI *WSARecv_fp_t)(SOCKET, LPWSABUF, DWORD, LPDWORD, LPDWORD,
                                  LPWSAOVERLAPPED,
                                  LPWSAOVERLAPPED_COMPLETION_ROUTINE);
typedef int(WINAPI *WSARecvFrom_fp_t)(SOCKET, LPWSABUF, DWORD, LPDWORD, LPDWORD,
                                      sockaddr *, LPINT, LPWSAOVERLAPPED,
                                      LPWSAOVERLAPPED_COMPLETION_ROUTINE);
typedef int(WINAPI *WSASend_fp_t)(SOCKET, LPWSABUF, DWORD, LPDWORD, DWORD,
                                  LPWSAOVERLAPPED,
                                  LPWSAOVERLAPPED_COMPLETION_ROUTINE);
typedef int(WINAPI *WSASendTo_fp_t)(SOCKET, LPWSABUF, DWORD, LPDWORD, DWORD,
                                    CONST sockaddr *, int, LPWSAOVERLAPPED,
                                    LPWSAOVERLAPPED_COMPLETION_ROUTINE);
typedef int(WINAPI *WSARecvMsg_fp_t)(SOCKET, LPWSAMSG, LPDWORD, LPWSAOVERLAPPED,
                                     LPWSAOVERLAPPED_COMPLETION_ROUTINE);
typedef int(WINAPI *WSASendMsg_fp_t)(SOCKET, LPWSAMSG, DWORD, LPDWORD,
                                     LPWSAOVERLAPPED,
                                     LPWSAOVERLAPPED_COMPLETION_ROUTINE);
typedef int(WINAPI *WSAPoll_fp_t)(LPWSAPOLLFD, ULONG, INT);
typedef DWORD(WINAPI *WSAWaitForMultipleEvents_fp_t)(DWORD, CONST HANDLE *,
                                                     BOOL, DWORD, BOOL);
typedef BOOL(WINAPI *GetQueuedCompletionStatus_fp_t)(HANDLE, LPDWORD,
                                                     PULONG_PTR, LPOVERLAPPED *,
                                                     DWORD);
typedef BOOL(WINAPI *GetQueuedCompletionStatusEx_fp_t)(HANDLE,
                                                       LPOVERLAPPED_ENTRY,
                                                       ULONG, PULONG, DWORD,
                                                       BOOL);
...
template <typename T> struct ioctl_param;

template <typename X, typename Y> struct ioctl_param<int (*)(X, Y, ...)> {
  typedef Y type;
};
...
typedef ssize_t (*recvfrom_fp_t)(int, void *, size_t, int, struct sockaddr *,
                                 socklen_t *);
...
typedef ssize_t (*sendto_fp_t)(int, const void *, size_t, int,
...
typedef int (*gethostbyname_r_fp_t)(const char *, struct hostent *, char *,
                                    size_t, struct hostent **, int *);
typedef int (*gethostbyname2_r_fp_t)(const char *, int, struct hostent *,
                                     char *, size_t, struct hostent **, int *);
typedef int (*gethostbyaddr_r_fp_t)(const void *, socklen_t, int,
                                    struct hostent *, char *, size_t,
                                    struct hostent **, int *);
#else
typedef int (*kevent_fp_t)(int, const struct kevent *, int, struct kevent *,
