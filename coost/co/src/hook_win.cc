...
public:
  ...
  void set_send_timeout(uint32_t ms) {
    _s.send_timeout = ms <= 65535 ? (uint16_t)ms : 65535;
  }
  void set_recv_timeout(uint32_t ms) {
    _s.recv_timeout = ms <= 65535 ? (uint16_t)ms : 65535;
  }

  int send_timeout() const {
    return _s.send_timeout == 0 ? -1 : _s.send_timeout;
  }
  int recv_timeout() const {
    return _s.recv_timeout == 0 ? -1 : _s.recv_timeout;
  }
  ...
  void set_shut_read() {
    if (atomic_or(&_s.flags, f_shut_read, mo_acq_rel) & f_shut_write)
      this->clear();
  }

  void set_shut_write() {
    if (atomic_or(&_s.flags, f_shut_write, mo_acq_rel) & f_shut_read)
      this->clear();
  }
...
inline coost::co::Hook &gHook() {
  static auto h = coost::mem::_make_static<coost::co::Hook>();
  return *h;
}
...
inline void set_skip_iocp(sock_t s, coost::co::HookCtx *ctx) {
  if (coost::co::can_skip_iocp_on_success && !ctx->has_skip_iocp()) {
    ctx->set_skip_iocp();
    SetFileCompletionNotificationModes((HANDLE)s,
...
SOCKET WINAPI hook_socket(int a0, int a1, int a2) {
...
SOCKET WINAPI hook_WSASocketA(int a0, int a1, int a2, LPWSAPROTOCOL_INFOA a3,
                              GROUP a4, DWORD a5) {
...
SOCKET WINAPI hook_WSASocketW(int a0, int a1, int a2, LPWSAPROTOCOL_INFOW a3,
                              GROUP a4, DWORD a5) {
  const bool ol = !!(a5 & WSA_FLAG_OVERLAPPED);
...
int WINAPI hook_shutdown(SOCKET a0, int a1) {
  ...
  case SD_RECEIVE:
    ctx->set_shut_read();
    r = coost::co::shutdown(a0, 'r');
    break;
  case SD_SEND:
    ctx->set_shut_write();
    r = coost::co::shutdown(a0, 'w');
    break;
  case SD_BOTH:
    ctx->clear();
    r = coost::co::shutdown(a0, 'b');
    break;
  default:
...
int WINAPI hook_setsockopt(SOCKET a0, int a1, int a2, const char *a3, int a4) {
...
int WINAPI hook_ioctlsocket(SOCKET a0, long a1, u_long *a2) {
...
int WINAPI hook_WSAIoctl(SOCKET a0, DWORD a1, LPVOID a2, DWORD a3, LPVOID a4,
  ...
  if (!sched || !ctx || ctx->is_non_blocking() ||
      (ctx->is_overlapped() && a7) || a1 == FIONREAD || a1 == SIOCATMARK) {
...
int WINAPI hook_WSAAsyncSelect(SOCKET a0, HWND a1, u_int a2, long a3) {
...
int WINAPI hook_WSAEventSelect(SOCKET a0, WSAEVENT a1, long a2) {
...
SOCKET WINAPI hook_accept(SOCKET a0, sockaddr *a1, int *a2) {
  SOCKET r;
  auto ctx = gHook().get_hook_ctx(a0);
  const auto sched = coost::co::xx::gSched;
  if (!sched || !ctx || ctx->is_non_blocking()) {
...
SOCKET WINAPI hook_WSAAccept(SOCKET a0, sockaddr *a1, LPINT a2,
                             LPCONDITIONPROC a3, DWORD_PTR a4) {
  SOCKET r;
  auto ctx = gHook().get_hook_ctx(a0);
  const auto sched = coost::co::xx::gSched;
  if (!sched || !ctx || ctx->is_non_blocking()) {
...
int WINAPI hook_connect(SOCKET a0, CONST sockaddr *a1, int a2) {
  int r;
  auto ctx = gHook().get_hook_ctx(a0);
  const auto sched = coost::co::xx::gSched;
  if (!sched || !ctx || ctx->is_non_blocking()) {
  ...
  if (ctx->is_overlapped()) {
    r = coost::co::connect(a0, a1, a2, ctx->send_timeout());
    goto end;
  }

  {
    ...
    while (true) {
      r = coost::co::getsockopt(a0, SOL_SOCKET, SO_CONNECT_TIME, &sec, &len);
      if (r != 0)
        goto end; // r = SOCKET_ERROR
      if (sec >= 0)
        goto end; // r = 0, connected
      if (t == 0) {
...
int WINAPI hook_WSAConnect(SOCKET a0, sockaddr *a1, int a2, LPWSABUF a3,
                           LPWSABUF a4, LPQOS a5, LPQOS a6) {
  int r;
  auto ctx = gHook().get_hook_ctx(a0);
  const auto sched = coost::co::xx::gSched;
  if (!sched || !ctx || ctx->is_non_blocking()) {
...
static LPWSABUF check_wsabufs(LPWSABUF p, DWORD n, int do_memcpy) {
  ...
  LPWSABUF x = (LPWSABUF)coost::mem::alloc(sizeof(WSABUF) * n);
  for (DWORD i = 0; i < n; ++i) {
    if (sched->on_stack(p[i].buf)) {
      x[i].buf = (char *)coost::mem::alloc(p[i].len);
      if (do_memcpy)
        memcpy(x[i].buf, p[i].buf, p[i].len);
    } else {
      x[i].buf = p[i].buf;
    }
    x[i].len = p[i].len;
...
static void clean_wsabufs(LPWSABUF x, LPWSABUF p, DWORD n, int do_memcpy) {
  for (DWORD i = 0; i < n; ++i) {
    if (x[i].buf != p[i].buf) {
      if (do_memcpy)
        memcpy(p[i].buf, x[i].buf, x[i].len);
      coost::mem::free(x[i].buf, x[i].len);
    }
  }
  coost::mem::free(x, sizeof(WSABUF) * n);
}
...
int WINAPI hook_recv(SOCKET a0, char *a1, int a2, int a3) {
  ...
  if (ctx->is_overlapped()) {
    coost::co::io_event ev(a0, coost::co::ev_read, a1, a2);
    ev->flags = a3;
    set_skip_iocp(a0, ctx);

    r = __sys_api(WSARecv)(a0, &ev->buf, 1, &ev->n, &ev->flags, &ev->ol, 0);
    if (r == 0) {
      if (!coost::co::can_skip_iocp_on_success)
        ev.wait();
    } else if (WSAGetLastError() == WSA_IO_PENDING) {
      if (!ev.wait(ctx->recv_timeout()))
        goto end; // r = -1
    } else {
      goto end;
    }
...
int WINAPI hook_WSARecv(SOCKET a0, LPWSABUF a1, DWORD a2, LPDWORD a3,
                        LPDWORD a4, LPWSAOVERLAPPED a5,
    ...
    if (r == 0) {
      *a3 = ev->n;
      if (a4)
        *a4 = ev->flags;
      if (x != a1)
        clean_wsabufs(x, a1, a2, 1);
      goto end;
    } else {
      *a3 = 0;
      if (x != a1)
        clean_wsabufs(x, a1, a2, 0);
      goto end;
    }
...
int WINAPI hook_recvfrom(SOCKET a0, char *a1, int a2, int a3, sockaddr *a4,
                         int *a5) {
  int r;
  auto ctx = gHook().get_hook_ctx(a0);
  const auto sched = coost::co::xx::gSched;
  ...
  if (ctx->is_overlapped()) {
    ...
    coost::co::io_event ev(a0, coost::co::ev_read, a1, a2, N);
    ev->flags = a3;
    set_skip_iocp(a0, ctx);
    ...
    if (N > 0) {
      ...
      *(int *)s = sizeof(SOCKADDR_STORAGE);
      r = __sys_api(WSARecvFrom)(a0, &ev->buf, 1, &ev->n, &ev->flags,
                                 (sockaddr *)(s + 8), (int *)s, &ev->ol, 0);
    } else {
      r = __sys_api(WSARecvFrom)(a0, &ev->buf, 1, &ev->n, &ev->flags, 0, 0,
                                 &ev->ol, 0);
    }
...
int WINAPI hook_WSARecvFrom(SOCKET a0, LPWSABUF a1, DWORD a2, LPDWORD a3,
                            LPDWORD a4, sockaddr *a5, LPINT a6,
                            LPWSAOVERLAPPED a7,
                            LPWSAOVERLAPPED_COMPLETION_ROUTINE a8) {
  int r;
  auto ctx = gHook().get_hook_ctx(a0);
  ...
  if (ctx->is_overlapped()) {
    ...
    if (N > 0) {
      s = ev->s;
      *(int *)s = sizeof(SOCKADDR_STORAGE);
      r = __sys_api(WSARecvFrom)(a0, x, a2, &ev->n, &ev->flags,
                                 (sockaddr *)(s + 8), (int *)s, &ev->ol, 0);
    } else {
      r = __sys_api(WSARecvFrom)(a0, x, a2, &ev->n, &ev->flags, 0, 0, &ev->ol,
                                 0);
    }
...
int WINAPI hook_send(SOCKET a0, CONST char *a1, int a2, int a3) {
  int r;
  auto ctx = gHook().get_hook_ctx(a0);
  const auto sched = coost::co::xx::gSched;
  if (!sched || !ctx || ctx->is_non_blocking() || a2 < 0) {
  ...
  if (ctx->is_overlapped()) {
    coost::co::io_event ev(a0, coost::co::ev_write, a1, a2);
    set_skip_iocp(a0, ctx);
...
int WINAPI hook_WSASend(SOCKET a0, LPWSABUF a1, DWORD a2, LPDWORD a3, DWORD a4,
                        LPWSAOVERLAPPED a5,
                        LPWSAOVERLAPPED_COMPLETION_ROUTINE a6) {
  int r;
  auto ctx = gHook().get_hook_ctx(a0);
...
int WINAPI hook_sendto(SOCKET a0, CONST char *a1, int a2, int a3,
                       CONST sockaddr *a4, int a5) {
  int r;
  auto ctx = gHook().get_hook_ctx(a0);
  const auto sched = coost::co::xx::gSched;
  if (!sched || !ctx || ctx->is_non_blocking() || a2 < 0) {
...
int WINAPI hook_WSASendTo(SOCKET a0, LPWSABUF a1, DWORD a2, LPDWORD a3,
                          DWORD a4, CONST sockaddr *a5, int a6,
                          LPWSAOVERLAPPED a7,
                          LPWSAOVERLAPPED_COMPLETION_ROUTINE a8) {
  int r;
  auto ctx = gHook().get_hook_ctx(a0);
...
static LPWSAMSG check_wsamsg(LPWSAMSG p, char c, int do_memcpy) {
  ...
  if (!mos && !cos && buf == p->lpBuffers && (!aos || c == 's'))
    return p;

  LPWSAMSG x = (LPWSAMSG)coost::mem::alloc(sizeof(*p));
  memcpy(x, p, sizeof(*p));

  if (c == 'r' && aos) {
    x->name = (LPSOCKADDR)coost::mem::alloc(sizeof(SOCKADDR_STORAGE));
    x->namelen = sizeof(SOCKADDR_STORAGE);
  }

  if (cos) {
    x->Control.buf = (char *)coost::mem::alloc(p->Control.len);
    if (do_memcpy)
      memcpy(x->Control.buf, p->Control.buf, p->Control.len);
  }
...
static void clean_wsamsg(LPWSAMSG x, LPWSAMSG p, int do_memcpy) {
  if (x->name != p->name) {
    if (do_memcpy && x->namelen <= p->namelen)
      memcpy(p->name, x->name, x->namelen);
    coost::mem::free(x->name, sizeof(SOCKADDR_STORAGE));
  }
  p->namelen = x->namelen;

  if (x->Control.buf != p->Control.buf) {
    if (do_memcpy)
      memcpy(p->Control.buf, x->Control.buf, x->Control.len);
    coost::mem::free(x->Control.buf, p->Control.len);
  }
  ...
  coost::mem::free(x, sizeof(*p));
}

int WINAPI hook_WSARecvMsg(SOCKET a0, LPWSAMSG a1, LPDWORD a2,
                           LPWSAOVERLAPPED a3,
...
int WINAPI hook_WSASendMsg(SOCKET a0, LPWSAMSG a1, DWORD a2, LPDWORD a3,
                           LPWSAOVERLAPPED a4,
...
int WINAPI hook_select(int a0, fd_set *a1, fd_set *a2, fd_set *a3,
                       const timeval *a4) {
...
int WINAPI hook_WSAPoll(LPWSAPOLLFD a0, ULONG a1, INT a2) {
  int r;
  const auto sched = coost::co::xx::gSched;
  if (!sched || a2 == 0) {
...
DWORD WINAPI hook_WSAWaitForMultipleEvents(DWORD a0, CONST HANDLE *a1, BOOL a2,
                                           DWORD a3, BOOL a4) {
...
BOOL WINAPI hook_GetQueuedCompletionStatus(HANDLE a0, LPDWORD a1, PULONG_PTR a2,
                                           LPOVERLAPPED *a3, DWORD a4) {
  BOOL r;
  DWORD t = a4, x = 1;
  const auto sched = coost::co::xx::gSched;
  if (!sched) {
...
BOOL WINAPI hook_GetQueuedCompletionStatusEx(HANDLE a0, LPOVERLAPPED_ENTRY a1,
                                             ULONG a2, PULONG a3, DWORD a4,
                                             BOOL a5) {
  BOOL r;
  DWORD t = a4, x = 1;
  const auto sched = coost::co::xx::gSched;
...
#define attach_hook(x)                                                         \
  detour_attach(&(PVOID &)__sys_api(x), (PVOID)hook_##x, #x)
#define detach_hook(x)                                                         \
  detour_detach(&(PVOID &)__sys_api(x), (PVOID)hook_##x, #x)
...
void hook_sleep(bool x) {
  coost::atomic_store(&gHook().hook_sleep, x, coost::mo_release);
}
