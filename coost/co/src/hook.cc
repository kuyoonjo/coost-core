...
inline struct hostent *gHostEnt() {
  static auto &e = *coost::mem::_make_static<coost::vector<struct hostent>>(
      coost::co::sched_num());
  return &e[coost::co::xx::gSched->id()];
}

inline coost::co::mutex &gDnsMutex_t() {
  static auto &m = *coost::mem::_make_static<coost::vector<coost::co::mutex>>(
      coost::co::sched_num());
  return m[coost::co::xx::gSched->id()];
}

inline coost::co::mutex &gDnsMutex_g() {
  static auto m = coost::mem::_make_static<coost::co::mutex>();
  return *m;
}
...
#define hook_api(f)                                                            \
  if (!__sys_api(f)) {                                                         \
    void *origin = 0;                                                          \
    rebind_symbols((rebinding[1]){{#f, (void *)::_hook(f), (void **)&origin}}, \
                   1);                                                         \
    atomic_store(&__sys_api(f), origin, coost::mo_relaxed);                    \
  }
#else
#define _hook(f) f
#define _hook_api(f)                                                           \
  if (!__sys_api(f))                                                           \
  atomic_store(&__sys_api(f), dlsym(RTLD_NEXT, #f), coost::mo_relaxed)
...
#define VARG_8_GET(args)                                                       \
  ...
  {                                                                            \
    a1 = va_arg(args, intptr_t);                                               \
    a2 = va_arg(args, intptr_t);                                               \
    a3 = va_arg(args, intptr_t);                                               \
    a4 = va_arg(args, intptr_t);                                               \
    a5 = va_arg(args, intptr_t);                                               \
    a6 = va_arg(args, intptr_t);                                               \
    a7 = va_arg(args, intptr_t);                                               \
    a8 = va_arg(args, intptr_t);                                               \
  }

#define VARG_8_PARAMS a1, a2, a3, a4, a5, a6, a7, a8

int _hook(ioctl)(int fd, coost::co::ioctl_param<ioctl_fp_t>::type request,
                 ...) {
  _hook_api(ioctl);
...
int _hook(setsockopt)(int fd, int level, int optname, const void *optval,
                      socklen_t optlen) {
...
int _hook(shutdown)(int fd, int how) {
  ...
  case SHUT_RD:
    ctx->set_shut_read();
    r = coost::co::shutdown(fd, 'r');
    break;
  case SHUT_WR:
    ctx->set_shut_write();
    r = coost::co::shutdown(fd, 'w');
    break;
  case SHUT_RDWR:
    ctx->clear();
    r = coost::co::shutdown(fd, 'b');
    break;
  default:
...
int _hook(connect)(int fd, const struct sockaddr *addr, socklen_t addrlen) {
  ...
  r = coost::co::connect(fd, addr, addrlen, ctx->send_timeout());
...
int _hook(accept)(int fd, struct sockaddr *addr, socklen_t *addrlen) {
  ...
  {
    coost::co::io_event ev(fd, coost::co::ev_read);
    do {
      r = __sys_api(accept)(fd, addr, addrlen);
      if (r != -1)
        goto end;
...
ssize_t _hook(recvfrom)(int fd, void *buf, size_t len, int flags,
  ...
  {
    coost::co::io_event ev(fd, coost::co::ev_read);
    do_hook(__sys_api(recvfrom)(fd, buf, len, flags, addr, addrlen), ev,
            ctx->recv_timeout());
  }
...
ssize_t _hook(write)(int fd, const void *buf, size_t count) {
  ...
  {
    coost::co::io_event ev(fd, coost::co::ev_write);
...
ssize_t _hook(sendto)(int fd, const void *buf, size_t len, int flags,
  ...
  {
    coost::co::io_event ev(fd, coost::co::ev_write);
    do_hook(__sys_api(sendto)(fd, buf, len, flags, addr, addrlen), ev,
            ctx->send_timeout());
  }
...
int _hook(poll)(struct pollfd *fds, nfds_t nfds, int ms) {
  ...
  do {
    if (nfds == 1) {
      auto ctx = gHook().get_hook_ctx(fd);
      if (!ctx || !ctx->is_sock_or_pipe() || !ctx->is_non_blocking())
        break;

      if (fds[0].events == POLLIN) {
        if (!sched->add_io_event(fd, coost::co::ev_read))
          break;
      } else if (fds[0].events == POLLOUT) {
        if (!sched->add_io_event(fd, coost::co::ev_write))
          break;
      } else {
        break;
      }
...
int _hook(select)(int nfds, fd_set *rs, fd_set *ws, fd_set *es,
                  struct timeval *tv) {
  _hook_api(select);
...
int _hook(epoll_wait)(int epfd, struct epoll_event *events, int n, int ms) {
  ...
  {
    coost::co::io_event ev(epfd, coost::co::ev_read);
    if (!ev.wait(ms)) {
      r = 0;
      goto end;
    } // timeout
...
#ifdef SOCK_NONBLOCK
int _hook(accept4)(int fd, struct sockaddr *addr, socklen_t *addrlen,
                   int flags) {
  _hook_api(accept4);
...
int _hook(gethostbyname_r)(const char *name, struct hostent *ret, char *buf,
  ...
  coost::co::mutex_guard g(gDnsMutex_t());
  return __sys_api(gethostbyname_r)(name, ret, buf, len, res, err);
}

int _hook(gethostbyname2_r)(const char *name, int af, struct hostent *ret,
...
int _hook(gethostbyaddr_r)(const void *addr, socklen_t addrlen, int type,
  ...
  const auto sched = coost::co::xx::gSched;
  if (!sched)
    return __sys_api(gethostbyaddr_r)(addr, addrlen, type, ret, buf, len, res,
                                      err);
...
struct hostent *_hook(gethostbyname2)(const char *name, int af) {
  ...
  const auto sched = coost::co::xx::gSched;
  if (!sched || !name)
    return __sys_api(gethostbyname2)(name, af);

  coost::fastream fs(1024);
  ...
  int r = -1;
  while (true) {
    r = gethostbyname2_r(name, af, ent, (char *)(fs.data() + 8),
                         fs.capacity() - 8, &res, err);
...
#else
int _hook(kevent)(int kq, const struct kevent *c, int nc, struct kevent *e,
...

  {
    ...
    {
      coost::co::io_event ev(kq, coost::co::ev_read);
      if (!ev.wait(ms)) {
        r = 0;
        goto end;
      }
...
struct hostent *_hook(gethostbyname)(const char *name) {
  ...
  const auto sched = coost::co::xx::gSched;
  if (!sched)
    return __sys_api(gethostbyname)(name);

  coost::co::mutex_guard g(gDnsMutex_g());
...
struct hostent *_hook(gethostbyaddr)(const void *addr, socklen_t len,
  ...
  const auto sched = coost::co::xx::gSched;
  if (!sched)
    return __sys_api(gethostbyaddr)(addr, len, type);
...
static bool _init_hook() {
  ...
  if (!__sys_api(select)) {
    void *origin = 0;
    rebind_symbols((rebinding[1]){{"select$1050", (void *)::_hook(select),
                                   (void **)&origin}},
                   1);
...
void hook_sleep(bool x) { atomic_store(&gHook().hook_sleep, x, mo_release); }
