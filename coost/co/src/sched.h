...
// DEC_uint32_t(co_sched_num);
// DEC_uint32_t(co_stack_num);
...
class Sched;
struct Coroutine;
typedef coost::multimap<int64_t, Coroutine *>::iterator timer_id_t;
...
inline waitx_t *make_waitx(void *co, size_t n = sizeof(waitx_t)) {
  waitx_t *w = (waitx_t *)coost::mem::alloc(n);
  assert(w);
  w->next = w->prev = 0;
  w->co = (Coroutine *)co;
  w->state = st_wait;
...
struct Buffer {
  ...
  void append(const void *p, size_t size) {
    ...
    if (_h->cap < _h->size + n) {
      ...
      _h->cap += (o >> 1) + n;
      _h = (H *)coost::mem::realloc(_h, o + 8, _h->cap + 8);
      assert(_h);
      goto lable;
    }
...
private:
  ...
  coost::array<Coroutine *> _v;
  coost::array<int> _use_count;
  coost::array<int> _v0; // id of coroutine in _v[0]
  coost::array<int> _vc; // id of coroutine in _v[_c]
  coost::set<int> _blks; // blocks available
};

// Task may be added from any thread. We need a mutex here.
class alignas(coost::mem::cache_line_size) TaskManager {
public:
  ...
  void get_all_tasks(coost::array<Closure *> &new_tasks,
                     coost::array<Coroutine *> &ready_tasks) {
    std::lock_guard<std::mutex> g(_mtx);
    if (!_new_tasks.empty())
      _new_tasks.swap(new_tasks);
    if (!_ready_tasks.empty())
      _ready_tasks.swap(ready_tasks);
  }

private:
  std::mutex _mtx;
  coost::array<Closure *> _new_tasks;
  coost::array<Coroutine *> _ready_tasks;
};
...
public:
  ...
  timer_id_t add_timer(uint32_t ms, Coroutine *co) {
    return _it = _timer.emplace_hint(_it, coost::time::now::ms() + ms, co);
  }
  ...
  uint32_t check_timeout(coost::array<Coroutine *> &res);

private:
  coost::multimap<int64_t, Coroutine *> _timer; // timed-wait tasks: <time_ms, co>
  coost::multimap<int64_t, Coroutine *>::iterator
      _it; // make insert faster with this hint
};
...
public:
  ...
  int64_t cputime() { return atomic_load(&_cputime, mo_relaxed); }

  // start the scheduler thread
  void start() { std::thread(&Sched::loop, this).detach(); }

  // stop the scheduler thread
  void stop(uint32_t ms = -1);
...
private:
  ...
  void save_stack(Coroutine *co) {
    if (co) {
      if (!co->pbuf && !_bufs.empty())
        co->pbuf = _bufs.pop_back();
      co->buf.clear();
...
private:
  union {
    int64_t _cputime;
    char _c0[coost::mem::cache_line_size];
  };
  union {
    struct {
      co::sync_event ev;
    ...
    char _c1[coost::mem::cache_line_size];
  };
...
public:
  ...
  const coost::vector<Sched *> &scheds() const { return _scheds; }

  void stop(uint32_t ms = (uint32_t)-1);

private:
  std::function<Sched *(const coost::vector<Sched *> &)> _next;
  coost::vector<Sched *> _scheds;
};
