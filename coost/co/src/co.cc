...
inline void cv_wait(cv_t *c, mutex_t *m) {
  SleepConditionVariableCS(c, m, INFINITE);
}
inline bool cv_wait(cv_t *c, mutex_t *m, uint32_t ms) {
...
uint32_t thread_id() { return GetCurrentThreadId(); }
...
bool cv_wait(cv_t *c, mutex_t *m, uint32_t ms) {
#ifdef __linux__
  struct timespec ts;
...
public:
  mutex() {
    const int r = pthread_mutex_init(&_m, 0);
    (void)r;
    assert(r == 0);
  }
  ~mutex() {
    const int r = pthread_mutex_destroy(&_m);
    (void)r;
    assert(r == 0);
  }

  void lock() {
    const int r = pthread_mutex_lock(&_m);
    (void)r;
    assert(r == 0);
  }
  void unlock() {
    const int r = pthread_mutex_unlock(&_m);
    (void)r;
    assert(r == 0);
  }
...
public:
  struct queue {
    ...
    ~queue() {
      for (auto h = _q.front(); h;) {
        ...
        h = h->next;
        mem::free(m, sizeof(_memb) + N * sizeof(void *));
      }
    }
  ...
  uint32_t unref() { return atomic_dec(&_refn, mo_acq_rel); }
...
public:
  event_impl(bool m, bool s, uint32_t wg = 0)
      : _wt(0), _sn(0), _refn(1), _wg(wg), _signaled(s), _manual_reset(m),
        _has_cv(false) {}
  ~event_impl() {
    if (_has_cv)
      xx::cv_free(&_cv);
  ...
  void ref() { atomic_inc(&_refn, mo_relaxed); }
  uint32_t unref() { return atomic_dec(&_refn, mo_acq_rel); }
  uint32_t *wg() noexcept { return &_wg; }
  ...
  if (sched) { /* in coroutine */
    Coroutine *co = sched->running();
    {
      ...
      while (!_wc.empty()) {
        ...
        if (w->state != st_timeout)
          break;
        _wc.pop_front();
        !x ? (void)(x = w) : mem::free(w, sizeof(*w));
      }
    ...
    sched->yield();
    if (!sched->timeout())
      mem::free(co->waitx, sizeof(waitx_t));
    co->waitx = nullptr;
    ...
    if (!_wc.empty()) {
      ...
      if (!has_wt) {
        do {
          ...
          h = h->next;
          if (atomic_bool_cas(&w->state, st_wait, st_ready, mo_relaxed,
                              mo_relaxed)) {
            has_wc = true;
...
public:
  ...
  ~sync_event_impl() { xx::cv_free(&_cv); }
...
class pipe_impl {
public:
  pipe_impl(uint32_t buf_size, uint32_t blk_size, uint32_t ms, pipe::C &&c,
            pipe::D &&d)
      : _buf_size(buf_size), _blk_size(blk_size), _ms(ms), _has_cv(false),
        _c(std::move(c)), _d(std::move(d)), _rx(0), _wx(0), _refn(1), _full(0),
        _closed(0) {
    _buf = (char *)mem::alloc(_buf_size);
  }

  ~pipe_impl() {
    mem::free(_buf, _buf_size);
    if (_has_cv)
      xx::cv_free(&_cv);
  }
  ...
  waitx *create_waitx(Coroutine *co, void *buf) {
    ...
    if (co && gSched->on_stack(buf)) {
      w = (waitx *)mem::alloc(sizeof(waitx) + _blk_size);
      w->buf = (char *)w + sizeof(waitx);
      w->len = sizeof(waitx) + _blk_size;
    } else {
      w = (waitx *)mem::alloc(sizeof(waitx));
      w->buf = buf;
      w->len = sizeof(waitx);
    }
    w->co = co;
...
void pipe_impl::read(void *p) {
  ...
  if (sched) {
    ...
    for (;;) {
      ...
      if (r || !atomic_bool_cas(&w->state, st_wait, st_timeout, mo_relaxed,
                                mo_relaxed)) {
...
void pipe_impl::close() {
  ...
  if (x == 0) {
    ...
    while (atomic_load(&_closed, mo_relaxed) != 2)
      co::sleep(1);
  }
}

pipe::pipe(uint32_t buf_size, uint32_t blk_size, uint32_t ms, pipe::C &&c,
           pipe::D &&d) {
  _p = mem::alloc(sizeof(pipe_impl), mem::cache_line_size);
  new (_p) pipe_impl(buf_size, blk_size, ms, std::move(c), std::move(d));
}

pipe::pipe(const pipe &p) : _p(p._p) {
  if (_p)
    god::cast<pipe_impl *>(_p)->ref();
}
...
class pool_impl {
public:
  typedef coost::array<void *> V;

  pool_impl() : _maxcap((size_t)-1), _refn(1) { this->_make_pools(); }

  pool_impl(std::function<void *()> &&ccb, std::function<void(void *)> &&dcb,
            size_t cap)
  ...
  void _make_pools() {
    _size = co::sched_num();
    _pools = (V *)mem::zalloc(sizeof(V) * _size);
  }

  void _free_pools() {
    for (int i = 0; i < _size; ++i)
      _pools[i].~V();
    mem::free(_pools, sizeof(V) * _size);
  }
...
mutex::mutex() {
  _p = mem::alloc(sizeof(xx::mutex_impl), mem::cache_line_size);
  new (_p) xx::mutex_impl();
}

mutex::mutex(const mutex &m) : _p(m._p) {
  if (_p)
    god::cast<xx::mutex_impl *>(_p)->ref();
}
...
mutex::~mutex() {
  ...
  if (p && p->unref() == 0) {
    p->~mutex_impl();
    mem::free(_p, sizeof(xx::mutex_impl));
    _p = 0;
  }
}
...
event::event(bool manual_reset, bool signaled) {
  _p = mem::alloc(sizeof(xx::event_impl), mem::cache_line_size);
  new (_p) xx::event_impl(manual_reset, signaled);
}

event::event(const event &e) : _p(e._p) {
  if (_p)
    god::cast<xx::event_impl *>(_p)->ref();
}
...
event::~event() {
  ...
  if (p && p->unref() == 0) {
    p->~event_impl();
    mem::free(_p, sizeof(xx::event_impl));
    _p = 0;
  }
}

bool event::wait(uint32_t ms) const {
...
sync_event::sync_event(bool manual_reset, bool signaled) {
  _p = mem::alloc(sizeof(xx::sync_event_impl), mem::cache_line_size);
...
sync_event::~sync_event() {
  if (_p) {
    ...
    mem::free(_p, sizeof(xx::sync_event_impl));
    _p = 0;
  }
}
...
bool sync_event::wait(uint32_t ms) {
...
void wait_group::done() const {
  ...
  const uint32_t x = atomic_dec(e->wg(), mo_acq_rel);
  // CHECK(x != (uint32_t)-1);
  if (x == 0)
    e->signal();
}
...
void wait_group::wait() const {
  god::cast<xx::event_impl *>(_p)->wait((uint32_t)-1);
}

pool::pool() {
  _p = mem::alloc(sizeof(xx::pool_impl), mem::cache_line_size);
  new (_p) xx::pool_impl();
}

pool::pool(const pool &p) : _p(p._p) {
  if (_p)
    god::cast<xx::pool_impl *>(_p)->ref();
}
...
pool::~pool() {
  ...
  if (p && p->unref() == 0) {
    p->~pool_impl();
    mem::free(_p, sizeof(xx::pool_impl));
    _p = 0;
  }
}

pool::pool(std::function<void *()> &&ccb, std::function<void(void *)> &&dcb,
           size_t cap) {
  _p = mem::alloc(sizeof(xx::pool_impl));
