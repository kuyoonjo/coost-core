...
// DEF_uint32(co_sched_num, os::cpunum(), ">>#1 number of coroutine
// schedulers"); DEF_uint32(co_stack_num, 8, ">>#1 number of stacks per
// scheduler, must be power of 2"); DEF_uint32(co_stack_size, 1024 * 1024, ">>#1
// size of the stack shared by coroutines"); DEF_bool(co_sched_log, false, ">>#1
// print logs for coroutine schedulers");

static inline uint32_t FLG_co_sched_num = std::thread::hardware_concurrency();
static inline uint32_t FLG_co_stack_num = 8;
static inline uint32_t FLG_co_stack_size = 1024 * 1024;
...
Sched::Sched(uint32_t id, uint32_t sched_num, uint32_t stack_num,
             uint32_t stack_size)
  ...
  _x.epoll = mem::make<Epoll>(id);
  _x.stopped = false;
  _main_co = _co_pool.pop(); // id 0 is reserved for _main_co
  _main_co->sched = this;
  _stack = (Stack *)mem::zalloc(stack_num * sizeof(Stack));
}
...
Sched::~Sched() {
  ...
  _bufs.clear();
  mem::free(_stack, _stack_num * sizeof(Stack));
}

void Sched::stop(uint32_t ms) {
  if (atomic_swap(&_x.stopped, true, mo_acq_rel) == false) {
    _x.epoll->signal();
    ms == (uint32_t)-1 ? _x.ev.wait() : (void)_x.ev.wait(ms);
  }
}
...
void Sched::resume(Coroutine *co) {
  ...
  _running = co;
  if (s->p == 0) {
    s->p = (char *)mem::alloc(_stack_size);
  ...
  if (co->ctx == 0) {
    ...
    // SCHEDLOG << "resume co: " << co << " id: " <<  co->id << " stack: " <<
    // co->buf.size();
    if (s->co != co) {
      this->save_stack(s->co);
    ...
    from = tb_context_jump(co->ctx,
                           _main_co); // jump back to where yiled() was called
  }
...
void Sched::loop() {
  gSched = this;
  coost::array<Closure *> new_tasks(512);
  coost::array<Coroutine *> ready_tasks(512);
  time::Timer timer;
  ...
  while (!_x.stopped) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      if (atomic_bool_cas(&info->state, st_wait, st_ready, mo_relaxed,
                          mo_relaxed)) {
    ...
    do {
      ...
      if (!new_tasks.empty()) {
        ...
        if (c >= 8192 && s <= (c >> 1)) {
          coost::array<Closure *>(s).swap(new_tasks);
        }
        new_tasks.clear();
      }
...
uint32_t TimerManager::check_timeout(coost::array<Coroutine *> &res) {
  if (_timer.empty())
    return (uint32_t)-1;

  int64_t now_ms = time::now::ms();
  ...
  for (; it != _timer.end(); ++it) {
    ...
    if (!co->waitx) {
      ...
      if (atomic_bool_cas(&w->state, st_wait, st_timeout, mo_relaxed,
                          mo_relaxed)) {
        res.push_back(co);
      }
    }
  ...
  return _timer.empty() ? (uint32_t)-1
                        : (uint32_t)(_timer.begin()->first - now_ms);
}
...
struct SchedInfo {
  SchedInfo() : cputime(co::sched_num(), 0), seed(coost::rand()) {}
  coost::array<int64_t> cputime;
  uint32_t seed;
};

inline SchedInfo &sched_info() {
  static __thread SchedInfo *s = 0;
  return s ? *s : *(s = mem::_make_static<SchedInfo>());
}
...
SchedManager::SchedManager() {
  ...
  const uint32_t ncpu = std::thread::hardware_concurrency();
  ...
  if (n != 1) {
    ...
    _next = [](const coost::vector<Sched *> &v) { return v[0]; };
  }

  for (uint32_t i = 0; i < n; ++i) {
    Sched *sched = mem::_make_static<Sched>(i, n, m, s);
    if (i != 0 || !main_thread_as_sched())
      sched->start();
...
inline SchedManager *sched_man() {
  static auto s = mem::_make_static<SchedManager>();
  return s;
}

void SchedManager::stop(uint32_t ms) {
...
const coost::vector<co::Sched *> &scheds() {
  return (coost::vector<co::Sched *> &)xx::sched_man()->scheds();
}

int sched_num() {
  return xx::is_active() ? (int)xx::sched_man()->scheds().size()
