...
inline void set_skip_iocp_on_success(sock_t fd) {
  if (can_skip_iocp_on_success) {
    SetFileCompletionNotificationModes((HANDLE)fd,
...
sock_t socket(int domain, int type, int protocol) {
  ...
  sock_t fd =
      __sys_api(WSASocketW)(domain, type, protocol, 0, 0, WSA_FLAG_OVERLAPPED);
  if (fd != (sock_t)-1) {
...
inline int get_address_family(sock_t fd) {
  ...
  if (r || info.iAddressFamily <= 0)
    COOST_LOG_FATAL("get address family failed, fd: ", fd);
...
sock_t accept(sock_t fd, void *addr, int *addrlen) {
  ...
  r = ::setsockopt(connfd, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char *)&fd,
                   sizeof(fd));
  if (r != 0) {
    e = WSAGetLastError();
    COOST_LOG_ERROR("acceptex set SO_UPDATE_ACCEPT_CONTEXT failed, sock: ",
                    connfd);
    goto err;
  }

  get_accept_ex_addrs(ev->s, 0, N + 16, N + 16, &serv, &serv_len, &peer,
                      &peer_len);
  if (addr && addrlen) {
    if (peer_len <= *addrlen)
...
int connect(sock_t fd, const void *addr, int addrlen, int ms) {
  ...
  do {
    ...
    if (co::bind(fd, &a, addrlen) != 0 && WSAGetLastError() != WSAEINVAL) {
      COOST_LOG_ERROR("connectex bind local address failed, sock: ", fd);
      return -1;
    }
  } while (0);
  ...
  if (r != 0) {
    COOST_LOG_ERROR("connectex set SO_UPDATE_CONNECT_CONTEXT failed, sock: ",
                    fd);
    goto err;
  }
  ...
  if (r == 0) {
    if (seconds < 0) {
      COOST_LOG_ERROR("dragon here, connectex getsockopt(SO_CONNECT_TIME), "
                      "seconds < 0, sock: ",
                      fd);
      goto err;
    }
    ...
    COOST_LOG_ERROR("connectex getsockopt(SO_CONNECT_TIME) failed, sock: ", fd);
    goto err;
  }
...
int recvfrom(sock_t fd, void *buf, int n, void *addr, int *addrlen, int ms) {
  ...
  if (N > 0) {
    ...
    *(int *)s = sizeof(SOCKADDR_STORAGE);
    r = __sys_api(WSARecvFrom)(fd, &ev->buf, 1, &ev->n, &ev->flags,
                               (sockaddr *)(s + 8), (int *)s, &ev->ol, 0);
  } else {
    r = __sys_api(WSARecvFrom)(fd, &ev->buf, 1, &ev->n, &ev->flags, 0, 0,
                               &ev->ol, 0);
  }
...
int sendto(sock_t fd, const void *buf, int n, const void *addr, int addrlen,
  ...
  do {
    r = __sys_api(WSASendTo)(fd, &ev->buf, 1, &ev->n, 0, (const sockaddr *)addr,
                             addrlen, &ev->ol, 0);
    if (r == 0) {
      if (!can_skip_iocp_on_success)
        ev.wait();
    ...
    if (ev->n > 0 && ev->n < (DWORD)ev->buf.len) {
      ...
      COOST_LOG_ERROR("dragon here, sendto ev->n: ", ev->n, ", n: ", n,
                      ", sock: ", fd);
      return -1;
    }
  } while (true);
...
bool _can_skip_iocp_on_success() {
  ...
  for (int i = 0; i < r; ++i) {
    if (!(proto_info[i].dwServiceFlags1 & XP1_IFS_HANDLES))
      return false;
  }
  return true;
}
...
void init_sock() {
  ...
  r = WSAIoctl(fd, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(guid),
               &connect_ex, sizeof(connect_ex), &n, 0, 0);
  if (r)
    COOST_LOG_FATAL("get ConnectEx failed: ", coost::strerror());

  guid = WSAID_ACCEPTEX;
  r = WSAIoctl(fd, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(guid),
               &accept_ex, sizeof(accept_ex), &n, 0, 0);
  if (r)
    COOST_LOG_FATAL("get AcceptEx failed: ", coost::strerror());

  guid = WSAID_GETACCEPTEXSOCKADDRS;
  r = WSAIoctl(fd, SIO_GET_EXTENSION_FUNCTION_POINTER, &guid, sizeof(guid),
               &get_accept_ex_addrs, sizeof(get_accept_ex_addrs), &n, 0, 0);
  if (r)
    COOST_LOG_FATAL("get GetAccpetExSockAddrs failed: ", coost::strerror());
