...
io_event::~io_event() {
  if (_added)
    xx::gSched->del_io_event(_fd, _ev);
}
...
io_event::~io_event() {
  if (!_timeout) {
    if (_to && _info->n > 0)
      memcpy(_to, _info->buf.buf, _info->n);
    coost::mem::free(_info, _info->mlen);
  }
...
bool io_event::wait(uint32_t ms) {
  ...
  if (_nb_tcp != 0) {
    if (_nb_tcp == nb_tcp_recv) {
      r = __sys_api(WSARecv)(_fd, &_info->buf, 1, &_info->n, &_info->flags,
                             &_info->ol, 0);
      if (r == 0 && can_skip_iocp_on_success)
        return true;
      if (r == -1) {
        e = WSAGetLastError();
        if (e != WSA_IO_PENDING)
      ...
      if (r == -1) {
        e = WSAGetLastError();
        if (e == WSAENOTCONN)
          goto wait_for_connect; // the socket is not connected yet
        if (e != WSA_IO_PENDING)
          goto err;
      }
    }
