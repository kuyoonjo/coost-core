...
sock_t socket(int domain, int type, int protocol) {
  return __sys_api(socket)(domain, type | SOCK_NONBLOCK | SOCK_CLOEXEC,
                           protocol);
}
...
int listen(sock_t fd, int backlog) { return ::listen(fd, backlog); }
...
#ifdef SOCK_NONBLOCK
    sock_t connfd =
        __sys_api(accept4)(fd, (sockaddr *)addr, (socklen_t *)addrlen,
                           SOCK_NONBLOCK | SOCK_CLOEXEC);
    if (connfd != -1)
      return connfd;
#else
    sock_t connfd =
        __sys_api(accept)(fd, (sockaddr *)addr, (socklen_t *)addrlen);
    if (connfd != -1) {
...
int recvfrom(sock_t fd, void *buf, int n, void *addr, int *addrlen, int ms) {
  ...
  io_event ev(fd, ev_read);
  do {
    int r = (int)__sys_api(recvfrom)(fd, buf, n, 0, (sockaddr *)addr,
                                     (socklen_t *)addrlen);
    if (r != -1)
      return r;
...
int sendto(sock_t fd, const void *buf, int n, const void *addr, int addrlen,
  ...
  do {
    int r = (int)__sys_api(sendto)(fd, p, remain, 0, (const sockaddr *)addr,
                                   (socklen_t)addrlen);
    if (r == remain)
      return n;
