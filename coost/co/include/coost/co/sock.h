...
/**
 * close a socket
 *   - In co 2.0.0 or before, a socket MUST be closed in the same thread that
 * performed the I/O operation. Since co 2.0.1, a socket can be closed anywhere.
 *   - EINTR has been handled internally. Users need not consider about it.
 *
 *   - NOTE: On Linux, if reference count of a socket is not zero when it was
 * closed, events will not be removed from epoll, which may cause a bug. That
 * may happen when users duplicated a socket by dup or dup2 and closed one of
 * them. At this case, users MUST close the socket in the thread performed the
 * I/O operation, and events will be removed from epoll by the scheduler then.
 *
...
/**
 * shutdown a socket
 *   - It is better to call shutdown() in the same thread that performed the I/O
 * operation.
 *
 * @param fd  a non-blocking (also overlapped on windows) socket.
 ...
 * @param fd       a non-blocking (also overlapped on windows) socket.
 * @param addr     a pointer to struct sockaddr, sockaddr_in or sockaddr_in6.
 * @param addrlen  the user MUST initialize it with the size of the structure
 * pointed to by addr; on return it contains the actual size of the peer
 * address.
 *
 * @return         a non-blocking (also overlapped on windows) socket on
 ...
 *             default: -1.
 *
 * @return     bytes recieved on success, -1 on timeout or error, 0 will be
 * returned if fd is a stream socket and the peer has closed the connection.
 */
 ...
 *   - It MUST be called in a coroutine.
 *   - It blocks until all the n bytes are recieved or timeout, or any error
 * occured.
 *
 * @param fd   a non-blocking (also overlapped on windows) socket,
 ...
 *   - It MUST be called in a coroutine.
 *   - It blocks until any data recieved or timeout, or any error occured.
 *   - Set src_addr and addrlen to NULL if the user is not interested in the
 * source address.
 *
 * @param fd        a non-blocking (also overlapped on windows) socket.
 * @param buf       a pointer to the buffer to recieve the data.
 * @param n         size of the buffer.
 * @param src_addr  a pointer to struct sockaddr, sockaddr_in or sockaddr_in6,
 * the source address will be placed in the buffer pointed to by src_addr.
 * @param addrlen   it should be initialized to the size of the buffer pointed
 * to by src_addr, on return, it contains the actual size of the source address.
 * @param ms        timeout in milliseconds, if ms < 0, it will never time out.
 *                  default: -1.
 *
 * @return          bytes recieved on success, -1 on timeout or error, 0 will be
 * returned if fd is a stream socket and the peer has closed the connection.
 */
int recvfrom(sock_t fd, void *buf, int n, void *src_addr, int *addrlen,
             int ms = -1);

/**
 * send n bytes on a socket
 *   - It MUST be called in a coroutine.
 *   - It blocks until all the n bytes are sent or timeout, or any error
 * occured.
 *
 * @param fd   a non-blocking (also overlapped on windows) socket.
 ...
 * @param fd        a non-blocking (also overlapped on windows) socket.
 * @param buf       a pointer to a buffer of the data to be sent.
 * @param n         size of the data, n MUST be no more than 65507 if fd is an
 * udp socket.
 * @param dst_addr  a pointer to struct sockaddr, sockaddr_in or sockaddr_in6,
 * which contains the destination address, SHOULD be NULL if fd is a tcp socket.
 ...
 */
int sendto(sock_t fd, const void *buf, int n, const void *dst_addr, int addrlen,
           int ms = -1);
...
inline int getsockopt(sock_t fd, int lv, int opt, void *optval, int *optlen) {
  ...
  if (r == 0)
    return r;
  coost::error() = WSAGetLastError();
  return r;
}

// set options on a socket, man setsockopt for details.
inline int setsockopt(sock_t fd, int lv, int opt, const void *optval,
                      int optlen) {
