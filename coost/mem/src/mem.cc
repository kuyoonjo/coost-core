...
#define WIN32_LEAN_AND_MEAN
    ...
    defined(__amd64)
#define _AMD64_
#elif defined(__i386) || defined(__i686) || defined(__i386__) ||               \
...
inline void _vm_decommit(void *p, size_t n) { VirtualFree(p, n, MEM_DECOMMIT); }

inline void _vm_free(void *p, size_t n) { VirtualFree(p, 0, MEM_RELEASE); }
...
inline uint32_t _find_lsb(size_t x) { /* x != 0 */
  unsigned long r;
...
inline uint32_t _pow2_align(uint32_t n) {
  unsigned long r;
...
inline void _vm_commit(void *p, size_t n) {
  void *x = ::mmap(p, n, PROT_READ | PROT_WRITE,
...
inline void _vm_decommit(void *p, size_t n) {
  (void)::mmap(p, n, PROT_READ | PROT_WRITE,
               MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE | MAP_FIXED, -1, 0);
}

inline void _vm_free(void *p, size_t n) { ::munmap(p, n); }
...
class MemBlocks {
public:
  explicit MemBlocks(uint32_t blk_size) : _m(0), _p(0), _blk_size(blk_size) {}

  ~MemBlocks() {
    if (_u) {
      for (uint32_t i = 0; i < _u[-2]; ++i)
        ::free(_m[i]);
      ::free(_u - 2);
    }
  }

  void *alloc(uint32_t n, uint32_t align = sizeof(void *));
  uint32_t size() const { return _u ? _u[-2] : 0; }
  uint32_t blk_size() const { return _blk_size; }
  uint32_t pos() const { return _p; }
  char *operator[](uint32_t i) const { return _m[i]; }
...
private:
  ...
  uint32_t _p;
  const uint32_t _blk_size;
};

void *MemBlocks::alloc(uint32_t n, uint32_t align) {
  if (unlikely(_m == 0)) {
    _u = (uint32_t *)::malloc(sizeof(char *) * 7 + 8) + 2;
  ...
  if (unlikely(p + n > x + _blk_size)) {
    if (_u[-2] == _u[-1]) {
      _u = (uint32_t *)::realloc(_u - 2, sizeof(char *) * _u[-1] * 2 + 8) + 2;
      _u[-1] *= 2;
    }
  ...
  _p = god::cast<uint32_t>(p - x) + n;
  return p;
}

class StaticAlloc {
public:
  StaticAlloc(uint32_t m) : _m(m) {}
  ~StaticAlloc() = default;

  void *alloc(size_t n) {
    const size_t H = coost::mem::cache_line_size >> 1;
    return _m.alloc((uint32_t)n,
                    n <= H ? sizeof(void *) : coost::mem::cache_line_size);
  }
...
class Dealloc {
public:
  static const uint32_t N = sizeof(F);
  Dealloc() : _m(8192) {}

  ~Dealloc() {
    const uint32_t M = _m.blk_size() / N;
    for (uint32_t i = _m.size(); i > 0; --i) {
      const uint32_t m = i != _m.size() ? M : _m.pos() / N;
      for (uint32_t x = m; x > 0; --x) {
...
#if __arch64
static const uint32_t B = 6;
static const uint32_t g_array_size = 32;
#else
static const uint32_t B = 5;
static const uint32_t g_array_size = 4;
#endif

static const uint32_t R = (1 << B) - 1;
static const size_t C = (size_t)1;
static const uint32_t g_sb_bits = 15;            // bit size of small block
static const uint32_t g_lb_bits = g_sb_bits + B; // bit size of large block
static const uint32_t g_hb_bits = g_lb_bits + B; // bit size of huge block
...
public:
  ...
  void set(uint32_t i) { _s[i >> B] |= (C << (i & R)); }

  void unset(uint32_t i) { _s[i >> B] &= ~(C << (i & R)); }

  bool test_and_unset(uint32_t i) {
    const size_t x = (C << (i & R));
    return god::fetch_and(&_s[i >> B], ~x) & x;
  }

  // find for a bit from MSB to LSB, starts from position @i
  int rfind(uint32_t i) const {
    int n = static_cast<int>(i >> B);
    do {
      const size_t x = _s[n];
      if (x)
        return _find_msb(x) + (n << B);
    } while (--n >= 0);
    return -1;
  }

  void atomic_set(uint32_t i) {
...
class HugeBlock : public coost::clink {
public:
  ...
  void *alloc() {
    const uint32_t i = _find_lsb(~_bits);
    if (i < R) {
      _bits |= (C << i);
  ...
  bool free(void *p) {
    const uint32_t i = (uint32_t)(((char *)p - _p) >> g_lb_bits);
    return (_bits &= ~(C << i)) == 0;
  }
...
class LargeBlock : public coost::clink {
public:
  ...
  bool free(void *p) {
    const uint32_t i = (uint32_t)(((char *)p - _p) >> g_sb_bits);
    return (_bits &= ~(C << i)) == 0;
  }
...
class LargeAlloc : public coost::clink {
public:
  static const uint32_t BS_BITS = 1u << (g_lb_bits - 12);
  static const uint32_t LA_SIZE = 64;
  static const uint32_t MAX_BIT = BS_BITS - 1;
  ...
  void *try_hard_alloc(uint32_t n);

  bool free(void *p) {
    int i = (int)(((char *)p - _p) >> 12);
    // CHECK(_bs.test_and_unset((uint32_t)i)) << "free invalid pointer: " << p;
    _bs.unset(i);
  ...
  void xfree(void *p) {
    const uint32_t i = (uint32_t)(((char *)p - _p) >> 12);
    _xbs.atomic_set(i);
  }

  void *realloc(void *p, uint32_t o, uint32_t n) {
    uint32_t i = (uint32_t)(((char *)p - _p) >> 12);
...
void *LargeAlloc::try_hard_alloc(uint32_t n) {
...
class SmallAlloc : public coost::clink {
public:
  static const uint32_t BS_BITS = 1u << (g_sb_bits - 4); // 2048
  static const uint32_t SA_SIZE =
      coost::mem::cache_line_size < 64 ? 64 : coost::mem::cache_line_size;
  static const uint32_t MAX_BIT = BS_BITS - ((SA_SIZE + (BS_BITS >> 2)) >> 4);
  ...
  explicit SmallAlloc(LargeBlock *parent, ThreadAlloc *ta)
    ...
    static_assert(coost::mem::cache_line_size <= (BS_BITS >> 3), "");
  ...
  void *alloc(uint32_t n, uint32_t a) {
    void *p = NULL;
    const uint32_t bit =
        (a <= (coost::mem::cache_line_size >> 4) || a <= (SA_SIZE >> 4))
            ? god::align_up(_bit, a)
            : god::align_up(_bit, a) +
                  (god::cast<uint32_t>(god::align_up(_p, a << 4) - _p) >> 4);
  ...
  void xfree(void *p) {
    const uint32_t i = (uint32_t)(((char *)p - _p) >> 4);
    _xbs.atomic_set(i);
  }

  void *realloc(void *p, uint32_t o, uint32_t n) {
    uint32_t i = (uint32_t)(((char *)p - _p) >> 4);
    if (_bit == i + o && i + n <= MAX_BIT) {
      _bit = i + n;
      return p;
    }
...
void *SmallAlloc::try_hard_alloc(uint32_t n) {
  size_t *const p = (size_t *)_pbs;
  size_t *const q = (size_t *)_xpbs;
...
public:
  ...
  void *alloc(uint32_t alloc_id, HugeBlock **parent);
  LargeBlock *make_large_block(uint32_t alloc_id);
  LargeAlloc *make_large_alloc(uint32_t alloc_id);
  void free(void *p, HugeBlock *hb, uint32_t alloc_id);
...
GlobalAlloc::~GlobalAlloc() {
  for (uint32_t i = 0; i < g_array_size; ++i) {
...
class alignas(coost::mem::cache_line_size) ThreadAlloc {
public:
  ThreadAlloc(GlobalAlloc *ga)
      : _lb(0), _la(0), _sa(0), _ga(ga), _s(16 * 1024) {
    static uint32_t g_alloc_id = (uint32_t)-1;
...
inline void *GlobalAlloc::alloc(uint32_t alloc_id, HugeBlock **parent) {
  void *p = NULL;
  ...
  if (p)
    _vm_commit(p, 1u << g_lb_bits);
  return p;
}

inline void GlobalAlloc::free(void *p, HugeBlock *hb, uint32_t alloc_id) {
  ...
  if (r)
    _vm_free(hb, 1u << g_hb_bits);
}

inline LargeBlock *GlobalAlloc::make_large_block(uint32_t alloc_id) {
  HugeBlock *parent;
  auto p = this->alloc(alloc_id, &parent);
  return p ? new (p) LargeBlock(parent) : NULL;
}

inline LargeAlloc *GlobalAlloc::make_large_alloc(uint32_t alloc_id) {
  HugeBlock *parent;
  auto p = this->alloc(alloc_id, &parent);
...
inline void *ThreadAlloc::alloc(size_t n) {
  ...
  SmallAlloc *sa;
  if (n <= 2048) {
    const uint32_t u = n > 16 ? god::nb<16>((uint32_t)n) : 1;
    if (_sa && (p = _sa->alloc(u)))
      goto end;
    ...
    const uint32_t u = god::nb<4096>((uint32_t)n);
    if (_la && (p = _la->alloc(u)))
      goto end;
...
inline void *ThreadAlloc::alloc(size_t n, size_t align) {
  ...
  SmallAlloc *sa;
  if (n <= 2048) {
    const uint32_t a = (uint32_t)align >> 4;
    const uint32_t u = n > 16 ? god::nb<16>((uint32_t)n) : 1;
    if (_sa && (p = _sa->alloc(u, a)))
      goto end;
...
inline void *ThreadAlloc::realloc(void *p, size_t o, size_t n) {
  if (unlikely(!p))
    return this->alloc(n);
  if (unlikely(o > g_max_alloc_size))
    return ::realloc(p, n);
  // CHECK_LT(o, n) << "realloc error, new size must be greater than old
  // size..";

  if (o <= 2048) {
    const uint32_t k = (o > 16 ? god::align_up<16>((uint32_t)o) : 16);
    if (n <= (size_t)k)
      return p;

    const auto sa = (SmallAlloc *)god::align_down<1u << g_sb_bits>(p);
    if (sa == _sa && n <= 2048) {
      const uint32_t l = god::nb<16>((uint32_t)n);
    ...
    const uint32_t k = god::align_up<4096>((uint32_t)o);
    if (n <= (size_t)k)
      return p;

    const auto la = (LargeAlloc *)god::align_down<1u << g_lb_bits>(p);
    if (la == _la && n <= g_max_alloc_size) {
      const uint32_t l = god::nb<4096>((uint32_t)n);
...
void *alloc(size_t n) { return xx::talloc()->alloc(n); }

void *alloc(size_t n, size_t align) { return xx::talloc()->alloc(n, align); }

void free(void *p, size_t n) { return xx::talloc()->free(p, n); }
