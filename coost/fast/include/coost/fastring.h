...
public:
  ...
  fastring(const void *s, size_t n) : fast::stream(n, n) { memcpy(_p, s, n); }

  fastring(size_t n, char c) : fast::stream(n, n) { memset(_p, c, n); }
  ...
  fastring(fastring &&s) noexcept : fast::stream(std::move(s)) {}
  ...
  template <typename S> fastring &assign(S &&s) {
  ...
  fastring &append(char c, size_t n) { return this->append(n, c); }

  fastring &append(char c) { return (fastring &)fast::stream::append(c); }

  fastring &append(signed char c) { return this->append((char)c); }

  fastring &append(unsigned char c) { return this->append((char)c); }
  ...
  template <typename X, typename... V> fastring &cat(X &&x, V &&...v) {
  ...
  fastring &operator<<(const fastring &s) { return this->append(s); }

  fastring &operator<<(const std::string &s) { return this->append(s); }

  bool contains(char c) const { return this->find(c) != npos; }

  bool contains(const char *s) const { return this->find(s) != npos; }

  bool contains(const fastring &s) const { return this->contains(s.c_str()); }
  ...
  bool ends_with(char c) const { return !this->empty() && this->back() == c; }

  bool ends_with(const char *s, size_t n) const {
    return n == 0 || (n <= _size && memcmp(_p + _size - n, s, n) == 0);
  }

  bool ends_with(const char *s) const { return this->ends_with(s, strlen(s)); }
  ...
  fastring &trim(signed char c, char d = 'b') { return this->trim((char)c, d); }
  ...
  fastring &trim(int n, char d = 'b') { return this->trim((size_t)n, d); }

  // the same as trim
  template <typename... X> fastring &strip(X &&...x) {
...
private:
  ...
  bool _inside(const char *p) const { return _p <= p && p < _p + _size; }
};

} // namespace coost

inline coost::fastring operator+(const coost::fastring &a, char b) {
  coost::fastring s(a.size() + 2);
  s.append(a).append(b);
  return s;
}

inline coost::fastring operator+(char a, const coost::fastring &b) {
  coost::fastring s(b.size() + 2);
  s.append(a).append(b);
  return s;
}

inline coost::fastring operator+(const coost::fastring &a,
                                 const coost::fastring &b) {
  coost::fastring s(a.size() + b.size() + 1);
  s.append(a).append(b);
  return s;
}
...
inline coost::fastring operator+(const std::string &a,
                                 const coost::fastring &b) {
  coost::fastring s(a.size() + b.size() + 1);
  s.append(a).append(b);
  return s;
...
inline coost::fastring operator+(const coost::fastring &a, const char *b) {
  const size_t n = strlen(b);
  coost::fastring s(a.size() + n + 1);
  s.append(a).append(b, n);
  return s;
}

inline coost::fastring operator+(const char *a, const coost::fastring &b) {
  const size_t n = strlen(a);
  coost::fastring s(b.size() + n + 1);
  s.append(a, n).append(b);
  return s;
}

inline bool operator==(const coost::fastring &a, const coost::fastring &b) {
  return a.size() == b.size() &&
         (a.empty() || memcmp(a.data(), b.data(), a.size()) == 0);
}

inline bool operator==(const coost::fastring &a, const std::string &b) {
  return a.size() == b.size() &&
         (a.empty() || memcmp(a.data(), b.data(), a.size()) == 0);
}

inline bool operator==(const std::string &a, const coost::fastring &b) {
  return b == a;
}

inline bool operator==(const coost::fastring &a, const char *b) {
  return a.size() == strlen(b) &&
         (a.empty() || memcmp(a.data(), b, a.size()) == 0);
}

inline bool operator==(const char *a, const coost::fastring &b) {
  return b == a;
}
...
inline bool operator<(const coost::fastring &a, const char *b) {
  const size_t n = strlen(b);
  return a.size() < n ? (a.empty() || memcmp(a.data(), b, a.size()) <= 0)
                      : (memcmp(a.data(), b, n) < 0);
}
...
inline bool operator<(const char *a, const coost::fastring &b) { return b > a; }
...
inline std::ostream &operator<<(std::ostream &os, const coost::fastring &s) {
  return os.write(s.data(), s.size());
}

namespace std {
template <> struct hash<coost::fastring> {
  size_t operator()(const coost::fastring &s) const {
    return coost::crypto::murmur_hash(s.data(), s.size());
  }
};
} // namespace std
...
public:
  ...
  anystr(const coost::fastring &s) noexcept : _s(s.data()), _n(s.size()) {}
