...
fastring &fastring::replace(const char *sub, const char *to,
                            size_t maxreplace) {
  if (this->empty())
    return *this;
...
static bool _match(const char *e, const char *p) {
  ...
  for (; *p && *e;) {
    if (*p == '*')
      return _match(e, p + 1) || _match(e + 1, p);
    if (*p != '?' && *p != *e)
      return false;
    ++p, ++e;
  }
...
bool fastring::match(const char *p) const { return _match(this->c_str(), p); }
