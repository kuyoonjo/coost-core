...
static void init_itoh_table(uint16_t *p) {
  for (int i = 0; i < 256; ++i) {
    char *b = (char *)(p + i);
...
static void init_itoa_table(uint32_t *p) {
...
inline uint16_t *create_itoh_table() {
  static uint16_t itoh_table[256];
  init_itoh_table(itoh_table);
  return itoh_table;
}

inline uint32_t *create_itoa_table() {
  static uint32_t itoa_table[10000];
  init_itoa_table(itoa_table);
  return itoa_table;
}

inline uint16_t *get_itoh_table() {
  static uint16_t *itoh_table = create_itoh_table();
  return itoh_table;
}

inline uint32_t *get_itoa_table() {
  static uint32_t *itoa_table = create_itoa_table();
  return itoa_table;
}

int dtoa(double v, char *buf, int mdp) { return milo::dtoa(v, buf, mdp); }

int u32toh(uint32_t v, char *buf) {
  static uint16_t *itoh_table = get_itoh_table();
  uint16_t b[4], *p = b + 4;
...
int u32toa(uint32_t v, char *buf) {
  static uint32_t *itoa_table = get_itoa_table();
  uint32_t b[3], *p = b + 2;
